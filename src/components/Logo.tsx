...
export function Logo({ className = '', showText = true, size = 'md', src, alt, textColor }: LogoProps) {
  ...
  return (
    ...
    >
      <span
        className={`flex items-center justify-center rounded-full bg-[var(--color-brand-accent-500)] shadow-md overflow-hidden ${sizeClasses[size]}`}
      >
        {logoSrc ? (
          <img
            src={logoSrc}
          ...
          <svg
            ...
            style={{ color: 'var(--color-brand-accent-500)' }}
          >
            <defs>
              <linearGradient id="waveGradient" x1="0%" y1="0%" x2="100%" y2="100%">
                <stop offset="0%" stopColor="var(--color-brand-accent-500)" stopOpacity="0.85" />
                <stop offset="100%" stopColor="var(--color-brand-accent-500)" stopOpacity="0.55" />
              </linearGradient>
            </defs>
            <rect x="0" y="0" width="48" height="48" rx="12" fill="var(--color-brand-accent-500)" />
            <path
