...
const navItems = [
  ...
  { label: 'Products', href: '/dashboard/products', icon: Package, requiresElevated: true },
  ...
  { label: 'Readiness', href: '/dashboard/readiness', icon: Gauge },
...
export function DashboardSidebar({ className, role = null, profile = null }: DashboardSidebarProps) {
  ...
  const checkActive = (href: string, exact?: boolean) => (exact ? pathname === href : pathname?.startsWith(href));
  const isTeamActive = Boolean(pathname?.startsWith('/dashboard/team'));

  const companyLabel = profile?.company?.name?.trim() || profile?.companyName?.trim() || 'Personal account';
  ...
  const privilegedNav = role === 'OWNER' || role === 'ADMIN' || role === 'SUPERADMIN';
  const filteredNavItems = navItems.filter((item) => {
    if ((item.label === 'Reporting' || item.label === 'Compliance') && !privilegedNav) return false;
    if (item.requiresElevated && !privilegedNav) return false;
    return true;
  });
  ...
  return (
    <aside
      ...
      style={sidebarTheme === 'dark' ? ({ ['--color-brand-sidebar-text']: '#e2e8f0' } as CSSProperties) : undefined}
    >
        ...
        {!collapsed && profile && (
          <div className="space-y-2 text-left">
            <div className="flex items-center gap-3">
              <div
                className={`h-10 w-10 overflow-hidden rounded-full border text-center text-sm font-semibold ${sidebarTheme === 'dark' ? 'border-zinc-700 bg-zinc-800 text-zinc-300' : 'border-zinc-200 bg-white text-zinc-600'}`}
              >
                {companyLogoUrl ? (
              ...
              <p className={`text-sm font-semibold ${sidebarTheme === 'dark' ? 'text-zinc-200' : 'text-zinc-900'}`}>
                {companyLabel}
              </p>
            </div>
            <p
              className={`text-base font-bold ${sidebarTheme === 'dark' ? 'text-zinc-100' : ''}`}
              style={sidebarTheme === 'dark' ? {} : { color: 'var(--color-brand-logo-text)' }}
            >
              {buildDisplayName(profile ?? undefined)}
            </p>
            {positionLabel ? (
              <p
                className={`text-xs font-semibold uppercase tracking-[0.3em] ${sidebarTheme === 'dark' ? 'text-zinc-400' : 'text-zinc-500'}`}
              >
                {positionLabel}
              </p>
            ) : null}
          ...
          {(role === 'OWNER' || role === 'ADMIN' || role === 'SUPERADMIN') && (
            <div className={`space-y-[1px] pt-0 pl-0 ${sidebarTheme === 'dark' ? '' : 'border-t border-white/30'}`}>
              ...
              >
                ...
                {!collapsed && (
                  <span
                    className={`transition ${isTeamActive ? 'text-[var(--color-brand-contrast)]' : 'group-hover:text-[var(--color-brand-contrast)]'}`}
                  >
                    Team
                  </span>
                )}
