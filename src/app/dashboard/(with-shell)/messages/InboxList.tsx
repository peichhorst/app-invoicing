...
const getContextKey = (msg: MessageRow) => {
  if (!msg.contextType || !msg.contextId) return 'GENERAL';
...
export default function InboxList({ messages, currentUserId, contextMetaByKey, usersById }: Props) {
  ...
  const rows = useMemo(() => localMessages, [localMessages]);
  ...
  return (
    <div className="divide-y divide-zinc-100">
      {rows.map((msg) => {
        ...
        const rowTone = isSender ? 'bg-brand-primary-50' : 'bg-emerald-50';
        ...
        return (
          <div key={msg.id} className="group">
            ...
            <details className="border-t border-zinc-100 sm:hidden">
              <summary className={`flex cursor-pointer items-center gap-3 px-4 py-3 ${rowTone}`}>
