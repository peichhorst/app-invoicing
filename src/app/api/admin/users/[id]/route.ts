...
export async function PATCH(request: Request) {
  ...
  // Only ADMIN can edit an OWNER
  if (target.role === 'OWNER' && user.role !== 'ADMIN') {
    return NextResponse.json({ error: 'Only admins can edit owners' }, { status: 403 });
  }

  const body = await request.json();
  const { name, positionId, email, phone, logoDataUrl, reportsToId } = body;
...
export async function DELETE(request: Request) {
  ...
  // Only ADMIN can delete an OWNER (and should cascade their entire company)
  if (target.role === 'OWNER' && user.role !== 'ADMIN') {
